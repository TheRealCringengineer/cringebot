...
def lookup_description(value):
    if value <= 0:
        ...
        return "Программист‑шаблонист, который уже умеет создавать шаблоны, но часто «перетаскивает» чужие без правок."
    elif value <= 63:
        return "Пастер‑переключатель, который уже умеет переключать ветки, но часто «копирует» чужие коммиты."
    elif value <= 64:
        return "Тот, кто уже умеет писать скрипты деплоя, но часто «переписывает» их из чужих CI‑конфигов."
    elif value <= 65:
        return "Ребят, купите мой софт. Реально не ратка"
    elif value <= 67:
        return "Пастер‑коктейль, который уже умеет смешивать технологии, но почти полностью состоит из чужих ингредиентов."
    elif value <= 68:
        return "Тот, кто уже умеет писать скрипты автоматизации, но часто «копирует» готовые задачи из чужих репо."
    elif value <= 69:
        ...
        return "Пастер‑субботник, который уже умеет «убирать» баги, но почти полностью «переписывает» чужие патчи."
    elif value <= 92:
