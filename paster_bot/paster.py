...
def lookup_description(value):
    ...
    lst = ["Держу почти весь рынок платных читов на себе","Имею дохуя репутации на StackOverflow","Я сам код пишу, а пасту ебашу чисто для души!","Каждый баг — это возможность спастить что-то новое","Да вообще я всё знаю, просто забыл","Даже цикл for копирую его из Stack Overflow.","Робин гуд от мира кода","Пастор вечного кода, чьи молитвы — это \"копировать\" и \"вставить\"","Если в репозитории нет чужого кода, значит, я где‑то ошибся","Нейросеть пишет код, а я только проверяю, что всё ещё компилируется","Если у меня есть ошибка, значит, я ещё не скопировал нужный кусок кода","Мой рефакторинг — это просто поиск новых мест, куда можно вставить чужой код","Ну мам, это пацаны пастили, а я просто рядом стоял.","Garbage collector успел собрать мой оригинальный код. Придётся пастить","Дефолтный пользователь UnknownCheats","Магистр копипасты, который верит в силу Ctrl+C и Ctrl+V","Программист без собственной фантазии, но с гигантским буфером обмена","TODO: Спастить смешное описание из ChatGPT","Великий исследователь чужих репозиториев","Ребят, купите мой софт. Реально не ратка","Мой проект: 70% копипаста, 30% \"почему это работает?\"","Если в проекте нет аимбота, значит я не нашёл, откуда его спиздить","Моя обфускация - это хуёвый код, сгенерированный нейронкой","Блять, софт сломался, пора создавать тему на югейме","Архитектура? А нахуя мне дома строить?","Сделал и продал OpenGL пасту на STALCRAFT","Всмысле софт спизжен? А юишку для лаунчера кто делал?","Ща ща ща. Сейчас я свою луашку запротекчу","Робин Гуд от мира кода.","Оптимизировал чит до O(1) – теперь он мгновенно крашит, потому что спизжен","Отредачил логи, типа сам писал","Как обойти бан на вайме?","Помогите спастить ноуклип","Шаблоны? Да, шаблонный чит, который я украл в первом попавшемся репозитории","Заменил строки с названием проекта и выдал за свой","Программист-коллекционер: всё копирую, всё собираю","Переименовал и продал OpenGL пасту на STALCRAFT","Писатель эпопеи, где каждый абзац — это строка из Stack Overflow","Дефолтный пользователь югейма"]
